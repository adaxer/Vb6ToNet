...
'''            Dim Federkonstante2 As Double
    ...
    'jetzt zur farblichen anzeige noch n paar spielereien
        ...
        Do
            ...
            Fuerstes = 0 + j * Memo
            Call Berechnung
            GoSub Kontrollrechnungen 'um die beurteilung komplett zu machen
...
'so, die rechnung stimmt, jetzt die restlichen aufgaben einfach erledigen
    ...
    GoSub Kontrollrechnungen 'noch fehlerwert und texte der gewählten konf. ermitteln
...
Dim i As Integer
    ...
    Do
        ...
        If K = ScheibeFedGew Then GoSub Wellenbelastung_abgleichen 'falls die antriebsscheibe betroffen ist
...
'besonderheiten feder/gewicht
Wellenbelastung_abgleichen:
        ...
        'sonst haben die beiden trums e etwa gleiche kräfte
            ...
            'auch falsch
                'If I = IaltK Then 'das band kommt vom höh. element
                '    P = Abs(Q + Tan(Memo / 180 * PI) * Q) + Abs(Q + Tan(Merk / 180 * PI) * M) 'sin bei 90° = 1, cos = 0
                'Else 'geht zum höheren element
                '    P = Abs(Cos(Memo / 180 * PI) * M) + Abs(Cos(Merk / 180 * PI) * Q)
                'End If
             
            'P = (Q + M) * Cos((Memo + Merk) / 2 * PI / 180) 'FR = (F1+F2)*cos (alpha/2), aus Kraftberechnung im parallelogram
...
'L soll die tatsächliche rechenreihenfolge (start im Leertrum) tarnen
    ...
    GoSub ALD 'ist schneller gefunden, wenn es in derselben prozedur ist
    ...
    Do
        ...
        If left(Sys(K).Tag, 1) = "0" Then 'einfach stehendes
            ...
            GoSub ALD 'kurve im element, wenn's kein träger ist
            'ImFörderer = False
        Else 'förderer
        ...
        'linie zum nächsten element ziehen
            ...
            GoSub ALD 'kurve zum nächsten element, wird immer durchgeführt
    ...
    'Schlupfausgleich an der Antriebsscheibe
        ...
        GoSub ALD 'schlupfausgleich über den durchmesser der antriebsscheibe
...
ALD: 'überdeckte Fu-fläche ermitteln, so, als wäre alles im positiven bereich
        'zum Zeichnen
